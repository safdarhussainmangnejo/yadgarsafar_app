...
export default function DashboardSidebar({ isOpenSidebar, onCloseSidebar }) {
  ...
  const { user } = useAuth();
  console.log("in DashboardSidebar UserOnly: ", user);
  const userData = user?.data;
  
  console.log("in DashboardSidebar User.Data: ", userData);
  
  
  const local = JSON.parse(localStorage.getItem('agency'));
  const local2 = JSON.parse(localStorage.getItem('user'));
      ...
      >
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Box component={RouterLink} to="/" sx={{ display: 'inline-flex' }}>
            ...
            <img src="/static/images/YadgarSafar.png" width={140} style={{ alignSelf: 'center' }} alt="yadgar-safar-logo"/>
          </Box>
